 ...
 *   - `header` header type, defaults to "Authorization", but can be set to any
 *              custom header name like "X-FooBar-Token". This is useful when there
 *              is a conflict between e.g. Basic Auth and Bearer Auth
 * Examples:
 *
...
Strategy.prototype.authenticate = function(req) {
  ...
  if (req.headers && req.headers[this._header]) {
    var parts = req.headers[this._header].split(' ');
    if (parts.length == 2) {
      var scheme = parts[0]
        , credentials = parts[1];
